...
for model_name in "${model_names[@]}"; do
    for activity_name in "${activity_names[@]}"; do
        ...
        CUDA_VISIBLE_DEVICES=4 python evaluate_using_mcqa_prompts.py --model_name "$model_name" --activity_name "$activity_name" --data_directory_path "../data/causal_query_triplets_sampled_instances_hard/" --shots 0 --prompt_template "random" --seed $seed
    done
done
